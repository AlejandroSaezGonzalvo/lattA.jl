...
elseif ens.id == "N302"
    ...
    m12 = get_mpcac(pp_sym[1], ap_sym[1], ens, "pion_wil", pl=false, wpm=wpm, tm=[[1], [12]], tM=[[10], [112]])
...
elseif ens.id == "E250"
    ...
    m13 = get_mpcac(pp_sym[2], ap_sym[2], ens, "kaon_wil", pl=false, wpm=wpm, tm=[[1], [7]], tM=[[10], [94]])
...
elseif ens.id == "D450"
    ...
    m12 = get_mpcac(pp_sym[1], ap_sym[1], ens, "pion_wil", pl=false, wpm=wpm, tm=[[1], [7,15,24]], tM=[[10], [62]])
    m13 = get_mpcac(pp_sym[2], ap_sym[2], ens, "kaon_wil", pl=false, wpm=wpm, tm=tm, tM=tM)
...
for i in 1:8:length(pp_tm)
    for j in 0:1
        ...
        elseif ens.id == "E300"
            ...
            push!(m34, m34_aux[1])
            fpi_aux = get_f_tm(pp_tm[i+j], mpi[end], ens, "pion_tm", wpm=wpm, tm=[[1], [44,50,60]], tM=[[10], [65,70,75,80]], pl=false)
            push!(fpi, fpi_aux[1])
        elseif ens.id == "J501"
            mpi_aux = get_m(pp_tm[i+j], ens, "pion_tm", wpm=wpm, tm=[[1], [62]], tM=[[10], [164]], pl=false)
            push!(mpi, mpi_aux[1])
            m12_aux = get_mpcac(pp_tm[i+j], ap_tm[i+j], ens, "pion_tm", wpm=wpm, tm=[[1], [30,40,50,60]], tM=[[10], [140,150,160]], pl=false)
            push!(m12, m12_aux[1])
            m34_aux = get_mpcac(pp_tm[i+j+6], ap_tm[i+j+6], ens, "pion_tm", wpm=wpm, tm=tm_m12[ens.id], tM=tM_m12[ens.id])
            push!(m34, m34_aux[1])
            fpi_aux = get_f_tm(pp_tm[i+j], mpi[end], ens, "pion_tm", wpm=wpm, tm=[[1], [40,63,77]], tM=[[10], [124,132,155,166]], pl=false)
            push!(fpi, fpi_aux[1])
        else
            mpi_aux = get_m(pp_tm[i+j], ens, "pion_tm", wpm=wpm, tm=tm_mpi[ens.id], tM=tM_mpi[ens.id], pl=false)
            push!(mpi, mpi_aux[1])
...
for i in 3:8:length(pp_tm)
    for j in 0:3
        println(i, " ", j)
        if ens.id == "N302"
            mk_aux = get_m(pp_tm[i+j], ens, "kaon_tm", wpm=wpm, tm=[[1], [23]], tM=[[10], [90,110]])
            push!(mk, mk_aux[1])
        ...
        elseif ens.id == "J501"
            ...
            push!(fk, fk_aux[1])
        elseif ens.id == "E300"
            mk_aux = get_m(pp_tm[i+j], ens, "kaon_tm", wpm=wpm, tm=[[1], [75,127]], tM=[[10], [167]], pl=false)
            push!(mk, mk_aux[1])
            m13_aux = get_mpcac(pp_tm[i+j], ap_tm[i+j], ens, "pion_tm", wpm=wpm, tm=tm_m13[ens.id], tM=tM_m13[ens.id])
            push!(m13, m13_aux[1])
