...
for i in 1:8:length(pp_sym)
    for j in 0:1
        ...
        push!(mpi, mpi_aux[1])
        m12_aux = get_mpcac(pp_sym[i+j], ap_sym[i+j], ens, "pion_tm", wpm=wpm, tm=[[55,57,59,61,65], [60,62,64,66]], tM=[[72,74,76], [70,72,74]])
        push!(m12, m12_aux[1])
        m34_aux = get_mpcac(pp_sym[i+j+6], ap_sym[i+j+6], ens, "pion_tm", wpm=wpm)
        push!(m34, m34_aux[1])
...
for i in 3:8:length(pp_sym)
    for j in 0:3
        ...
        push!(mk, mk_aux[1])
        m13_aux = get_mpcac(pp_sym[i+j], ap_sym[i+j], ens, "pion_tm", wpm=wpm)
        push!(m13, m13_aux[1])
