...
function get_m(corr::juobs.Corr, ens::EnsInfo; tm::Union{Vector{Int64}, Nothing}=nothing, tM::Union{Vector{Int64}, Nothing}=nothing, pl::Bool=false, wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
    corr_d = corr.obs
    ...
    m, syst, m_i, weight, pval = model_av([fit_const, fit_exp], m_dat, ens_db[ens.id][2], tm=tm, tM=tM, k=[k1,k2], wpm=wpm)
    if pl == true
