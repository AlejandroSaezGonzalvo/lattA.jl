...
function get_m_ALPHA(corr::juobs.Corr, ens::EnsInfo, PS::String; 
    tm::Int64=5,
    pl::Bool=false, wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
    ...
    @.fit_const(x,p) = p[1] * x ^ 0
    @.fit_excited(x,p) = p[2] * exp(-p[3] * (x-y0)) + p[4] * exp(-p[5] * (T-x))
    @.fit_exp(x,p) = p[1] + p[2] * exp(-p[3] * (x-y0)) + p[4] * exp(-p[5] * (T-x))
    k1 = 5
    ...
    i = ini = tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(i:div(T,2)), m_dat[i:div(T,2)], k1, guess, wpm=wpm)
        if pval > 0.1
            break
        end
        i+=1
    ...
    vec2 = fit_excited(x,up); uwerr.(vec2)
    tmin = x[minimum(findall(err.(vec) .> 4 * err.(vec2)))]
    #[vec vec2 err.(vec) .- 4 * err.(vec2) x]

    ## tmax
    pval = 0
    i = fin = T-tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(div(T,2):i), m_dat[div(T,2):i], k1, guess, wpm=wpm)
        if pval > 0.1
            break
        end
        i-=1
    ...
    vec = fit_exp(x,up); uwerr.(vec)
    vec2 = fit_excited(x,up); uwerr.(vec2)
    tmax = x[maximum(findall(err.(vec) .> 4 * err.(vec2)))]
    #[vec vec2 err.(vec) .- 4 * err.(vec2) x]

    m = juobs.plat_av(m_dat, [tmin, tmax])
    ...
    if pl == true
        ...
        savefig(string("/home/asaez/cls_ens/codes/lattA.jl/plots/m_",ens.id,"_",PS,"_plat_ALPHA.pdf"))
        #close("all")
    end
...
function get_m_pbc_ALPHA(corr::juobs.Corr, ens::EnsInfo, PS::String; 
    tm::Int64=5,
    pl::Bool=false, wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
    ...
    @.fit_fun(x,p) = p[2] * exp(-p[1] * (x-1)) + p[2] * exp(-p[1] * (T2+1-x))
    @.fit_excited(x,p) = p[3] * exp(-p[4] * (x-1)) + p[2] * exp(-p[4] * (T2+1-x))
    @.fit_fun2(x,p) = p[2] * exp(-p[1] * (x-1)) + p[2] * exp(-p[1] * (T2+1-x)) + p[3] * exp(-p[4] * (x-1)) + p[2] * exp(-p[4] * (T2+1-x))
    k1 = 2
    k2 = 4
    ...
    i = ini = tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_fun2, collect(i:div(T,2)), corr_d[i:div(T,2)], k2, guess, wpm=wpm)
        i+=1
        if pval > 0.1
            break
        end
    ...
    i = fin = T-tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_fun2, collect(div(T,2):i), corr_d[div(T,2):i], k2, guess, wpm=wpm)
        i-=1
        if pval > 0.1
            break
        end
...
function get_mpcac_ALPHA(corr_pp::juobs.Corr, corr_ap::juobs.Corr, ens::EnsInfo, PS::String; 
    impr::Bool=true, pl::Bool=false, tm::Int64=5,
    wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
    ...
    i = ini = tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(i:div(T,2)), mpcac_dat[i:div(T,2)], k1, guess, wpm=wpm)
        i+=1
        if pval > 0.1
            break
        end
    ...
    i = fin = T-tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(div(T,2):i), mpcac_dat[div(T,2):i], k1, guess, wpm=wpm)
        i-=1
        if pval > 0.1
            break
        end
    ...
    mpcac = juobs.plat_av(mpcac_dat, [tmin, tmax])
    ...
    if pl == true
        ...
        savefig(string("/home/asaez/cls_ens/codes/lattA.jl/plots/mpcac_",ens.id,"_",PS,"_plat_ALPHA.pdf"))
        #close("all")
    end
...
function get_f_wil_ALPHA(corr_pp::juobs.Corr, corr_ap::juobs.Corr, m::uwreal, ens::EnsInfo, PS::String;
    impr::Bool=true, pl::Bool=false, tm::Int64=5,
    wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
    ...
    @.fit_exp(x,p) = p[1] + p[2] * exp(-p[3] * (x-y0)) + p[4] * exp(-p[5] * (T-1-y0-x))
    @.fit_excited(x,p) = p[2] * exp(-p[3] * (x-y0)) + p[4] * exp(-p[5] * (T-1-y0-x))
    @.fit_const(x,p) = p[1] * x ^ 0
    k1 = 5
    k2 = 1
    ...
    i = ini = tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(i:div(T,2)), R_dat[i:div(T,2)], k1, guess, wpm=wpm)
        i+=1
        if pval > 0.1
            break
        end
    ...
    i = fin = T-tm
    while pval < 0.1
        global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(div(T,2):i), R_dat[div(T,2):i], k1, guess, wpm=wpm)
        i-=1
        if pval > 0.1
            break
        end
    ...
    R = juobs.plat_av(R_dat, [tmin, tmax])
    ...
    if pl == true
        ...
        savefig(string("/home/asaez/cls_ens/codes/lattA.jl/plots/R_",ens.id,"_",PS,"_plat_ALPHA.pdf"))
        #close("all")
    end
...
function get_f_wil_ALPHA(corr_ppL::juobs.Corr, corr_ppR::juobs.Corr, corr_apL::juobs.Corr, corr_apR::juobs.Corr, 
    m::uwreal, ens::EnsInfo, PS::String; pl::Bool=false, tm::Int64=5,
    wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
...
function get_f_tm_ALPHA(corr_pp::juobs.Corr, m::uwreal, ens::EnsInfo, PS::String;
    tm::Int64=5, pl::Bool=false, wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
...
function get_f_tm_ALPHA(corr_ppL::juobs.Corr, corr_ppR::juobs.Corr, m::uwreal, ens::EnsInfo, PS::String;
    tm::Int64=5, pl::Bool=false, wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing)
...
function get_w0(path::String, ens::EnsInfo, plat::Vector{Int64};
    ...
    t2YM, tdt2YM, W_obs, t = get_YM(paht, ens, rw=rw)
    t2YM_guess = [plat_av(t2YM[:,i], plat) for i in 1:length(t2YM[1,:])]
    nt0 = findmin(abs.(value.(t2YM_guess[2:end]) .- 0.3))[2]  
...
function get_t0_ALPHA(path::String, ens::EnsInfo, plat::Vector{Int64};
    pl::Bool=false, tm::Int64=5,
    rw=false, npol::Int64=2, ws::ADerrors.wspace=ADerrors.wsg, 
    wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing, 
    info::Bool=false)
    ...
    xx = t[nt0-dt0:nt0+dt0]
    t2E_i = Array{uwreal,1}()
    for j in 1:2*dt0+1
        i = Int(round(dt0+0.5))
        dat = Y_aux[:,j].* xx[j].^2 / L^3
        guess = value(dat[Int64(round(T / 2))])
        println(guess)
        if j == i   
            ...
            i = ini = tm
            while pval < 0.1
                global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(i:div(T,2)), dat[i:div(T,2)], k1, guess, wpm=wpm)
                i+=1
                if pval > 0.1
                    break
                end
            ...
            i = fin = T-tm
            while pval < 0.1
                global up, chi2, chi_exp, pval = fit_alg(fit_exp, collect(div(T,2):i), dat[div(T,2):i], k1, guess, wpm=wpm)
                i-=1
                if pval > 0.1
                    break
                end
            ...
            t2E_aux = juobs.plat_av(dat, [tmin, tmax])
            ...
            if pl == true
                ...
                savefig(string("/home/asaez/cls_ens/codes/lattA.jl/plots/t2E_",ens.id,"_plat_ALPHA.pdf"))
                #close("all")
            end
        else 
    ...
    par, aux = fit_alg(model, xx, t2E_i, npol)
    fmin(x, p) = model(x, p) .- 0.3
    t0 = root_error(fmin, t[nt0], par)
