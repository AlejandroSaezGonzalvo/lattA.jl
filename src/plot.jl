...
function matching_constTr_plot()
    ...
    ylabel(L"$am_{12}^{\rm (v)}$")  
    #xlabel(L"$1/\kappa^{\rm(v)}$")   
    vect = match_constTr([x_l; x_l; x_l], up)[1:length(xp[:,1])]
    uwerr.(vect)
    v = value.(vect)
    e = err.(vect)
    fill_between(1 ./ xp[:,1], v-e, v+e, color="orange", alpha=0.5)
    ...
    ylabel(L"$\phi_4^{\rm (v)}$")  
    xlabel(L"$1/\kappa^{\rm(v)}$")   
    vect = match_constTr([x_l; x_l; x_l], up)[2*length(xp[:,1])+1:3*length(xp[:,1])] .+ [phi4_ph for i in 1:length(xp[:,1])]
    uwerr.(vect)
    v = value.(vect)
    e = err.(vect)
    ...
    fill_between(1 ./ xp[:,1], v-e, v+e, color="orange", alpha=0.5)
    fill_between(1 ./ xp[:,1], v2-e2, v2+e2, color="gray", alpha=0.5)
    errorbar(1 ./ x[1:3:9,1], value.(phi4_sh[1:3:9]), err.(phi4_sh[1:3:9]), fmt="x", color="purple", label=L"$a\mu_l^{\rm(v)}=???$")
    errorbar(1 ./ x[2:3:9,1], value.(phi4_sh[2:3:9]), err.(phi4_sh[2:3:9]), fmt="x", color="green", label=L"$a\mu_l^{\rm(v)}=???$")
    errorbar(1 ./ x[3:3:9,1], value.(phi4_sh[3:3:9]), err.(phi4_sh[3:3:9]), fmt="x", color="blue", label=L"$a\mu_l^{\rm(v)}=???$")
    kappa_target = 1 / up[1]; uwerr(kappa_target)
    ...
    fill_between(xp[:,2], v-e, v+e, color="orange", alpha=0.5)
    fill_between(xp[:,2], v2-e2, v2+e2, color="gray", alpha=0.5)
    errorbar(x[1:3,2], value.(phi4_sh[1:3]), err.(phi4_sh[1:3]), fmt="x", color="purple", label=L"$a\mu_l^{\rm(v)}=???$")
    errorbar(x[4:6,2], value.(phi4_sh[4:6]), err.(phi4_sh[4:6]), fmt="x", color="green", label=L"$a\mu_l^{\rm(v)}=???$")
    errorbar(x[7:9,2], value.(phi4_sh[7:9]), err.(phi4_sh[7:9]), fmt="x", color="blue", label=L"$a\mu_l^{\rm(v)}=???$")
    mul_target = up[2]; uwerr(mul_target)
