...
\label{ch_ss:tab:stat}
	Contributions to $\sigma_{\textrm{stat}}^2$ in $\sqrt{t_0}$ [Combined] & \\
	\toprule
	Gauge ensembles & $75.56\%$ \\
	Renormalization and improvement & $1.0\%$ \\
	$V_{ud}$ & $0.01\%$ \\
	$V_{us}$ & $15.34\%$ \\
	QED corrections to $f_{\pi}$ & $0.4\%$ \\
	QED corrections to $f_K$ & $3.84\%$ \\
	Experimental $f_{\pi}$ & $0.01\%$ \\
	Experimental $f_K$ & $3.84\%$ \\
	Pion and Kaon meson masses & $<0.01\%$ \\
    \bottomrule
    \caption{Different contributions to statistical uncertainty for $\sqrt{t_0}$ [Combined] in eq.~(\ref{ch_ss:eq:t0ph_c}).}
\end{longtable}
