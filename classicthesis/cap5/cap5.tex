...
The final results for $\sqrt{t_0^{\textrm{ph}}}$ in physical units as computed from the model average for the different data sets, using $f_{\pi K}^{\textrm{isoQCD}}$ as physical inpuy, are
\begin{align}
\label{ch_ss:eq:t0ph}
...
As mentioned above, relying on external physical input of $f_K$ to set the scale is problematic due to uncertainties in the definition of $f_K^{\textrm{isoQCD}}$ related to QED corrections. Furthermore, the very experimental measurement of $f_K$ relies on information on the CKM matrix element $V_{us}$, and as such $f_K$ has a big uncertainty associated to this. This makes of particular importance to set the scale using only external physical input of $f_{\pi}$. To this end, we explore chiral fits to eqs.~(\ref{ch_ss:eq:SU2pi})-(\ref{ch_ss:eq:SU2k}), in addition to the different possibilities for the continuum limit and cuts in data introduced above. Though we fit both $\sqrt{t_0}f_{\pi}$ and $\sqrt{t_0}f_K$ simultaneously since they share a fit parameter in the chiral dependence, we extract $t_0^{\textrm{ph}}$ as
\begin{equation}
\sqrt{t_0^{\textrm{ph}}}=\frac{\left(\sqrt{t_0}f_{\pi}\right)^{\textrm{latt}}(\phi_2^{\textrm{ph}},a=0)}{f_{\pi}^{\textrm{isoQCD}}}.
\end{equation}
After model averaging we find the results
\begin{align}
...
\end{align}
We show the model average results in Figs.~\ref{ch_ss:fig:BMA_w_SU2}-\ref{ch_ss:fig:BMA_comb_SU2}, and the numerical values can be found in Appendix~\ref{apex_model_av_t0}. Finally, in Fig.~\ref{ch_ss:fig:SU2_comb} we show the particular case of one of the models entering the model average.
