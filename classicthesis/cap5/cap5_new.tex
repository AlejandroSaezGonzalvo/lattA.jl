...
The scale setting is the process of determining the values of the lattice spacing in physical units. It requires the use of physical input to set the bare parameters of the theory. An intermediate dimensionful scale is often used to construct dimensionless quantities out of lattice observables, which are naturally obtained in units of the lattice spacing. The scale setting procedure allows to determine the physical value of this intermediate scale. 
...
\end{equation}
After measuring $\sqrt{t_0}f_{\pi K}$ for each ensemble, one must perform a chiral-continuum extrapolation in order to extract its value at physical values of the quark masses and in the continuum. To define the physical point we use the pion and kaon physical masses, or more specifically the dimensionless quantities $\phi_2$ and $\phi_4$ in eqs.~(\ref{ch_ma:eq:phi2}-\ref{ch_ma:eq:phi4}). Thanks to the mass shifting procedure in Sec.~\ref{ch_ma:sec:chiral_traj}, the value of $\phi_4$ is kept fixed to its physical value along the chiral trajectory in the quark mass plane, and as a result the chiral extrapolation needs to be done in $\phi_2$ only. For the determination of the physical value of the latter we employ the iterative procedure explained at the end of Sec. \ref{ch_ma:sec:chiral_traj}. The latter relies on the physical input in eq.~(\ref{ch_ss:eq:isoQCD}) and an educated guess for the physical value of $t_0$, which is updated at each iterative step with the new determination of $t_0^{\textrm{ph}}$ coming from the scale setting analysis. Thus, with each iterative step both the values of $\phi_2$ to which we perform the chiral extrapolation and the value of $\phi_4$ to which we shift our observables are updated until convergence in $t_0^{\textrm{ph}}$ is obtained, which in practice occurs after a few iterative steps.
...
In order to perform the chiral-continuum limit extrapolation, we explore different ways of parameterizing the dependence on $\phi_2$ ($\phi_4$ is constant thanks to the mass shifting procedure of Sec.~\ref{ch_ma:sec:chiral_traj}) and on the lattice spacing $a$, and employ the model averaging techniques introduced in Sec.~\ref{ch_observables:sec:MA}.

After performing the chiral-continuum limit extrapolation, using as external physical input the values of the pion and kaon decay constants we can determine the value of the scale $t_0$ as
\begin{equation}
...
\end{align}
Thus, the combination $\sqrt{8t_0}f_{\pi K}$ reads
\begin{align}
...
The systematic uncertainty in the extraction of $\sqrt{t_0^{\textrm{ph}}}$ is assessed by the model variation using the TIC introduced in Sec.~\ref{ch_observables:sec:MA}. We vary over the different ways of performing the chiral-continuum limit extrapolation introduced above, as well as over the possibility of performing data cuts. In particular, we consider the following cuts (in addition to the ``no cut'' choice)
\begin{align}
\label{ch_ss:eq:cuts}
\beta&>3.40, \\
\beta&>3.46, \\
...
c_i^2=c_{\beta}^2\left(\frac{a^4}{t_0^2}\right)^2+c_{\phi_2}^2\left(\phi_2^2\right)^2,
\end{equation}
...
\label{ch_ss:tab:stat}
	...
	\toprule
	Model variation (systematic effects) & $35.9\%$ \\
	...
	Physical input for $f_{\pi}$ & $<0.01\%$ \\
	Physical input for $f_K$ & $1.64\%$ \\
...
The results in eqs.~(\ref{ch_ss:eq:t0ph_w}-\ref{ch_ss:eq:t0ph_c}) are the outcome of the iterative procedure explained at the end of Sec. \ref{ch_ma:sec:chiral_traj} and in the introduction to this Chapter. The convergence of the result of $\sqrt{t_0}$ using this iterative analysis is shown in Fig.~\ref{ch_ss:fig:iter}.
...
\begin{equation}
\label{ch_ss:eq:ratio}
\frac{\sqrt{t_0/a^2}}{\sqrt{t_0^{\textrm{sym}}/a^2}},
\end{equation}
where $\sqrt{t_0/a^2}$ is the measurement of the gradient flow scale in each ensemble while $\sqrt{t_0^{\textrm{sym}}/a^2}$ is the corresponding lattice determination, at the same value of the inverse coupling $\beta$, but using a symmetric point ensemble. Following~\citep{Strassberger:2023xnj} we fit the ratio in eq.~(\ref{ch_ss:eq:ratio}) to
\begin{equation}
...
\end{align}
Yet another alternative prescription is that recently proposed in the workshop ``Converging on QCD+QED prescriptions'' \citep{AP}, in which the isoQCD pion and kaon masses, in addition to the pion decay constant, are defined with no errors as
\begin{align}
\label{ch_ss:eq:AP}
